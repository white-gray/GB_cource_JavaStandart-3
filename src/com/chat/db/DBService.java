...
// создание таблицы и админа; да и ещё кучки пользователей
        ...
        rs = stmt.executeUpdate("INSERT INTO `gn_javast_less3_accounts`.`users`\n" +
                ...
                "`e-mail`,\n" +
                "`password`\n"+
                ")VALUES (\"admin\", \"adm@mail.com\", \"0\");");
        for (int j=1; j<21; j++){
        rs = stmt.executeUpdate("INSERT INTO `gn_javast_less3_accounts`.`users`\n" +
                "(`name`,\n" +
                "`e-mail`,\n" +
                "`password`\n"+
                ")VALUES (\""+j+"\", \""+j+"@mail.com\", \""+j+"\");");
        }
        stmt.close();
        return connection;
    }
 ...
 // проверка прописана ли в базе таблица
    ...
    public static boolean checkAccount(Connection connection, String [] test) throws SQLException {
...
//!!!!!            sendMessage("No a such user by nick and password.");
            return false;
        } catch (SQLException throwables) {
